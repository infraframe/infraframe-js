...
const ParticipantEvent = function(type, init) {
  const that = new EventModule.InfraframeEvent(type, init);
  /**
   * @member {Infraframe.Conference.Participant} participant
   * @instance
   * @memberof Infraframe.Conference.ParticipantEvent
   */
...
class ConferenceClientConfiguration {
  ...
  constructor() {
     ...
     * @instance
     * @memberof Infraframe.Conference.ConferenceClientConfiguration
 ...
 * | --------------------- | ---------------------------------| ---------------- |
 * | streamadded           | Infraframe.Base.StreamEvent             | A new stream is available in the conference. |
 * | participantjoined     | Infraframe.Conference.ParticipantEvent  | A new participant joined the conference. |
 * | messagereceived       | Infraframe.Base.MessageEvent            | A new message is received. |
 * | serverdisconnected    | Infraframe.Base.InfraframeEvent                | Disconnected from conference server. |
 *
 * @memberof Infraframe.Conference
 * @extends Infraframe.Base.EventDispatcher
 * @constructor
 * @param {?Infraframe.Conference.ConferenceClientConfiguration } config Configuration for ConferenceClient.
 * @param {?Infraframe.Conference.SioSignaling } signalingImpl Signaling channel implementation for ConferenceClient. SDK uses default signaling channel implementation if this parameter is undefined. Currently, a Socket.IO signaling channel implementation was provided as ics.conference.SioSignaling. However, it is not recommended to directly access signaling channel or customize signaling channel for ConferenceClient as this time.
 */
export const ConferenceClient = function(config, signalingImpl) {
   ...
   * @memberof Infraframe.Conference.ConferenceClient
  ...
  function onSignalingMessage(notification, data) {
    if (notification === 'soac' || notification === 'progress') {
      if (channels.has(data.id)) {
        ...
        quicTransportChannel.hasContentSessionId(data.id)
      ) {
  ...
  signaling.addEventListener('disconnect', () => {
    ...
    self.dispatchEvent(new EventModule.InfraframeEvent('serverdisconnected'));
  });
  ...
  function fireParticipantEvent(data) {
    if (data.action === 'join') {
      ...
      participant.dispatchEvent(new EventModule.InfraframeEvent('left'));
    }
  }
  ...
  function fireStreamRemoved(info) {
    ...
    const streamEvent = new EventModule.InfraframeEvent('ended');
  ...
  function updateRemoteStream(streamInfo) {
    ...
    stream.extraCapabilities =
      StreamUtilsModule.convertToSubscriptionCapabilities(streamInfo.media);
    const streamEvent = new EventModule.InfraframeEvent('updated');
    stream.dispatchEvent(streamEvent);
  }
  ...
  function createRemoteStream(streamInfo) {
    if (streamInfo.type === 'mixed') {
      ...
      let audioSourceInfo;
      let videoSourceInfo;
      const audioTrack = streamInfo.media.tracks.find(
          (t) => t.type === 'audio',
      );
      const videoTrack = streamInfo.media.tracks.find(
          (t) => t.type === 'video',
      );
      if (audioTrack) {
   ...
   * @memberof Infraframe.Conference.ConferenceClient
   * @return {Promise<ConferenceInfo, Error>} Return a promise resolved with current conference's information if successfully join the conference. Or return a promise rejected with a newly created Infraframe.Error if failed to join the conference.
          ...
          (resp) => {
            ...
            if (typeof WebTransport === 'function' && token.webTransportUrl) {
              quicTransportChannel = new QuicConnection(
                  token.webTransportUrl,
                  resp.webTransportToken,
                  createSignalingForChannel(),
                  config.webTransportConfiguration,
              );
            }
   ...
   * @param {Infraframe.Base.RemoteStream} stream The stream to be subscribed.
   * @param {Infraframe.Conference.SubscribeOptions} options Options for subscription.
