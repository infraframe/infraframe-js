...
let {
  ...
  { ConferenceClient, SioSignaling } = Owt.Conference;
...
let client = isSignaling
    ? new ConferenceClient(ccOptions, signaling)
...
function restListRooms() {
  listRooms(
    (rooms) => {
      ...
      rooms.forEach((item) => {
        if (item.name === "sampleRoom" && !defaultRoomId)
          defaultRoomId = item._id;
...
function restDropSipcalls() {
  let { roomId, sipcalld } = getRestfulParmas();
  deleteSipcalls(
    roomId,
    sipcalld,
    (resp) => {
...
function restUpdateSipcalls() {
  let { roomId, sipcalld, audioFrom, videoFrom } = getRestfulParmas();
  let { bitrate, resolution, frameRate, kfi } = getSubOptions();
...
function restForbidSub() {
  ...
  forbidSub(
    ...
    (resp) => {
      resp = JSON.parse(resp);
      console.log(
        `forbid participant ${participantId} subscribe ability success: `,
        resp
      );
    },
    (err) => {
      console.log(
        `forbid participant ${participantId} subscribe ability failed`,
        err
      );
    }
  );
...
function restForbidPub() {
  ...
  forbidPub(
    ...
    (resp) => {
      resp = JSON.parse(resp);
      console.log(
        `forbid participant ${participantId} publish ability success: `,
        resp
      );
    },
    (err) => {
      console.log(
        `forbid participant ${participantId} publish ability failed`,
        err
      );
    }
  );
...
function restSetRegion() {
  let { roomId, mixedStreamId, regionId, forwardStreamId } = getRestfulParmas();
  setRegion(
    roomId,
    mixedStreamId,
    regionId,
...
function restStartRecording() {
  let { roomId, audioFrom, videoFrom, container } = getRestfulParmas();
  let { videoCodec, audioCodec, bitrate, resolution, frameRate, kfi } =
    getSubOptions();
  let options = {
    media: {
      audio: false,
...
function restupdateRecording() {
  let { roomId, recorderId, audioFrom, videoFrom, audioActive, videoActive } =
    getRestfulParmas();
  let { bitrate, resolution, frameRate, kfi } = getSubOptions();
  let updateOptions = [];
...
function restStartAnalyzing() {
  let { roomId, audioFrom, videoFrom, algorithm } = getRestfulParmas();
  let { videoCodec, audioCodec, bitrate, resolution, frameRate, kfi } =
    getSubOptions();
  let options = {
    media: {
  ...
  if (options.media.video) {
    startAnalyzing(
      ...
      (resp) => {
        resp = JSON.parse(resp);
        console.log(
          `start analyzing ${audioFrom}, ${videoFrom} success: `,
          resp
        );
...
function restStopAnalyzing() {
  let { roomId, analyticsId } = getRestfulParmas();
  stopAnalyzing(
    roomId,
    analyticsId,
    (resp) => {
    ...
    videoCodecs.forEach(function (videoCodec) {
      ...
      startRecording(
        ...
        (resp) => {
          resp = JSON.parse(resp);
          console.log(
            `start recording ${audioFrom}, ${videoFrom} success: `,
            resp,
            "recording options",
            options
          );
...
function displayStream(stream, category = "forward") {
  ...
  if (category === "forward") {
    ...
    if (source === "camera" || source === "mic") {
      $("#localVideo video").attr("title", stream.id).get(0).srcObject =
        stream.mediaStream;
    } else {
      $("#screenVideo video").attr("title", stream.id).get(0).srcObject =
        stream.mediaStream;
    }
  }
}
      ...
      mix &&
        mixStream(
          ...
          (resp) => {
            resp = JSON.parse(resp);
            console.log(
              `mix stream ${remoteStream.id} to common success: `,
              resp
            );
          },
          (err) => {
            console.log(
              `mix stream ${remoteStream.id} to common failed: `,
              err
            );
          }
        );
...
function streamEvent(stream) {
  ...
  stream.addEventListener(
    ...
    activeaudioinputchangeListener
  );
  ...
  MediaStreamFactory.createMediaStream(mediaStreamDeviceConstraints)
    ...
    .then(
      (localStream) => {
        ...
        localStreams.set(
          localStream.source.video || localStream.source.audio,
          localStream
        );
...
function publish() {
  ...
  let { audioCodec, videoCodec } = getAudioAndVideoCodec(
    audioCodecName,
    videoCodecName
  );
  ...
  let stream = localStreams.get(
    localStream.source.video || localStream.source.audio
  );
  client.publish(stream, options).then(
    (publication) => {
      ...
      select.append(
        $(
          `<option value=${JSON.stringify(item)}>${
            item.name
              ? item.name +
                (item.profile === undefined ? "" : "-" + item.profile)
...
$("#availableremotestream").change(function () {
  ...
  let { audio, video } = remoteStream.capabilities;
  if (audio) {
    let { codecs: supportedAudioCodecs } = audio;
...
function getSubOptions() {
  ...
  let videoCodec = $("#supportedvideocodec").val()
    ? JSON.parse($("#supportedvideocodec").val())
    : undefined;
  let audioCodec = $("#supportedaudiocodec").val()
    ? JSON.parse($("#supportedaudiocodec").val())
    : undefined;
  let bitrate = $("#supportedbitrate").val()
    ? $("#supportedbitrate").val()
    : undefined;
  let resolution = $("#supportedresolution").val()
    ? JSON.parse($("#supportedresolution").val())
    : undefined;
  let frameRate = $("#supportedframerate").val()
    ? parseInt($("#supportedframerate").val())
    : undefined;
  let kfi = $("#supportedkfi").val()
    ? parseInt($("#supportedkfi").val())
    : undefined;
  let subscriptionId =
    $("#availablesubscription").val() ||
    $("#availablesubscription option:eq(1)").val();
  return {
    remoteStreamId,
    trackKind,
    videoCodec,
...
function subscribe() {
  ...
  client.subscribe(remoteStreamMap.get(remoteStreamId), subOptions).then(
    (subscription) => {
...
function sendMessage() {
  ...
  client.send(message, participantId).then(
    () => {
...
function publicationMute() {
  ...
  publicationArr.forEach((publication) => {
    publication.mute(trackKind).then(
      () => {
...
function publicationUnmute() {
  let [publicationArr, trackKind] = getPublicationParams();
  publicationArr.forEach((publication) => {
    publication.unmute(trackKind).then(
      () => {
...
function subscriptionApplyOption() {
  ...
  subscriptions
    ...
    .then(
      () => {
        console.log(
          `subscription ${subscriptionId} apply options success, options are: `,
          options
        );
      },
      (err) => {
...
function subscriptionChangeStream() {
  let { subscriptionId } = getSubOptions();
  let { forwardStreamId } = getRestfulParmas();
  let options = {
    video: {
      from: forwardStreamId,
    },
  };
...
function subscribeTest(apply) {
  ...
  remoteStreams.forEach((item, key) => {
    let { audio: audioCapabilities, video: videoCapabilities } =
      item.capabilities;
...
function mix_unmix() {
  ...
  let inter = setInterval(() => {
    mixStream(roomId, forwardStreamId, view, (resp) => {
      ...
      resp = JSON.parse(resp);
      console.log(
        `mix stream ${forwardStreamId} to ${view} success: `,
        resp,
        m
      );
      unmixStream(
        ...
        (resp) => {
          resp = JSON.parse(resp);
          console.log(
            `UNmix stream ${forwardStreamId} to ${view} success: `,
            resp,
            m
          );
        },
        (err) => {
          console.log(
            `UNmix stream ${forwardStreamId} to ${view} failed: `,
            err
          );
        },
        (err) => {
    ...
    .then((devicesInfos) => {
      devicesInfos.forEach((deviceInfo) => {
        if (deviceInfo.kind === "audioinput") {
          $("#audiodeviceid").append(
            `<option value=${deviceInfo.deviceId}>${deviceInfo.deviceId}</option>`
          );
        }
      });
    })
      ...
      (token) => {
        client.join(token).then(
          (resp) => {
            ...
            let streamConstraints = new StreamConstraints(
              mstdcForMic,
              mstdcForCamera
            );
            ...
            // });
            // console.log(
            //   "18c6ce227497a74b702601426972c7ffa149403fd2bd9a60a92be7c5244b29ba"
            // );
            ...
            //     frameRate: undefined,
            //     deviceId:
            //       "f3658f9ede2d13921631dd501ef6e3f90faccf05683e6d25c97360d87df6305f",
            //     source: "camera",
            //   },
            // })
            MediaStreamFactory.createMediaStream({
              ...
              .then(
                (mediaStream) => {
                  ...
                  return new LocalStream(
                    mediaStream,
                    new StreamSourceInfo(undefined, "camera"),
                    { attributes: "test att ributes" }
                  );
                },
                (err) => {
              ...
              .then(
                (localStream) => {
                  ...
                  displayStream(localStream, "local");
                  isPublish &&
                    client.publish(localStream, publishOputions).then(
                      (publication) => {
                        publications.set(publication.id, publication);
...
window.onbeforeunload = function (event) {
  ...
  if (localStream) {
    ...
    if (
      localStream.channel &&
      typeof localStream.channel.close === "function"
    ) {
